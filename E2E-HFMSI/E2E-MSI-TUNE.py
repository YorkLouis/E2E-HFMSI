...
path = os.path.abspath(r'C:/Users\Administrator\Desktop/E2E-HFMSI\data_MIS')
path1 = os.path.abspath(r'C:/Users\Administrator\Desktop/E2E-HFMSI/Result')
...
mode =  1 # 0：训练；1：测试；2：可解释性分析；3：Optuna超参数优化
epoch, lr, batch = 30, 0.015, 64  #40 30, 0.02
early_stop_epochs = 5 
tc = 0 #0, 0.0005, 0.001, 0.0015, 0.002, 0.0025, 0.003
...
# for tc in [0.0005, 0.0015, 0.0025 ]:  #0.0005, 0.001, 0.0015, 0.002, 0.0025, 0.003
#     print("tc:", tc)
for data_name in data_set:    
        ...
        if data_name == 'SSE50':
            ...
            n_i, n_s, start = 12, 47, 220 # epoch20
        ...
        if mode == 0:
                ...
                for e in range(epoch):
                    ...
                    for it, (data_all, lables) in enumerate(train_loader):
                        ...
                        sharpe = Object(bc, (lables.to(device)).transpose(1, 2), tc) - lambd * torch.norm(bc, p=2, dim=1,
                                                                                                    keepdim=False, out=None,
                                                                                                    dtype=None).mean()
                        # sharpe = (re-1).sum(1).mean()/torch.std((re-1).sum(1))#-0.5*torch.norm(bc, p=2, dim=1, keepdim=False, out=None, dtype=None).mean()
            ...
            with torch.no_grad():
                ...
                # ================= 敏感性分析开始 =================
                tc_sensitivity_list = [0, 0.0005, 0.001, 0.0015, 0.002, 0.0025, 0.003]#[0, 0.0005, 0.001, 0.0015, 0.002, 0.0025, 0.003]
            ...
            model.load_state_dict(torch.load(path1 + '/mis' + '_' + data_name + '_' + 'win' +
                            str(win) + 'lr' + str(lr) + 'ep' +
                            str(epoch) +'lam'+str(lambd)+ 'ab'+str(ablation)+'hid'+ str(hidden) + 'fd.pth', map_location=device))
